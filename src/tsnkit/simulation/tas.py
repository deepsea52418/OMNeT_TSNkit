...
# from ..utils import find_files_with_prefix, SEED
from  tsnkit.utils import  find_files_with_prefix, SEED
np.random.seed(SEED)
...
def simulation(
    ...
    print(" _path:= {}  \n"
          "_prefix:= {}  \n"
          "config_paths:= {} \n".format(_path, _prefix, config_paths))
    ...
    for link in GCL:
        ...
        for i, row in enumerate(temp[:-1]):  # type: ignore
            if row[1] > temp[i + 1][0]:
                print("gcl overlap", link, row, temp[i + 1])
    ...
    # 这里只统计了流count，好像没有使用到offset具体值
    OFFSET_MAX: Dict[int, int] = {}
    _df = offset.groupby("stream", as_index=False).count()
    ...
    for t in tqdm(range(0, hyper_period * it, T_SLOT)):
        ...
        for flow in range(len(task)):
            ...
            # 这个时刻恰好是flow的第instance_count[flow]个帧的出发时间（从0数起）
            if (t / period[flow] >= instance_count[flow]) and t % period[flow] == OFFSET[frame]:
...
if __name__ == "__main__":
    ...
    log = simulation(
        ...
        parser.parse_args().task, parser.parse_args().config, it=1, verbose=True
    )
