...
class smt_nw:
        ...
        for s in task:
            ...
            for link in path.iter_link():
                if link.src == s.src:
                    ...
                    delay[s][link] = delay[s][prev_link] + link.t_proc + s.get_t_trans(link)
        return delay
    ...
    def get_delay(self) -> utils.Delay:
        ...
        for s in self.task:
            ...
            # print('stream %s  ', str(s))
            # print('delay  %d  ', _delay)
            # print('release_time  %d  last_link_time %d   first_link_time %d '  %(self.release[s].x, self.delay[s][s.last_link], self.delay[s][s.first_link] ))
